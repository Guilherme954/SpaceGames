...
from .models import Jogo, Biblioteca
...
@login_required
def download_jogo(request, jogo_id):

    jogo = get_object_or_404(Jogo, id=jogo_id)

    possui = Biblioteca.objects.filter(
        usuario=request.user,
        jogo=jogo
    ).exists()
    ...
    if not possui:
        messages.error(
            request,
            "Você precisa possuir este jogo para baixar."
        )
        return redirect('detalhe_jogo', jogo_id=jogo.id)

    return HttpResponseRedirect(jogo.link_download)